#!/usr/bin/env python3
"""
Script para procesar los datos de rayos y convertirlos a formato JSON separado para visualización web.
Separa los datos de rayos y medio en dos archivos JSON diferentes.
"""
import json
import csv
import glob
...
def main():
    parser = argparse.ArgumentParser(description="Procesa datos de rayos para visualización web separada")
    ...
    # Crear estructura de datos para rayos
    ray_output_data = {
        "rays": rays_data,
        "endpoints": endpoints,
    ...
    # Crear estructura de datos para medio
    medium_output_data = {
        "parameters": params,
        "medium": medium_data,
        "stats": {
    ...
    # Guardar archivos JSON separados
    ray_file = os.path.join(args.output_dir, "ray_data.json")
    medium_file = os.path.join(args.output_dir, "medium_data.json")
    
    with open(ray_file, "w") as f:
        json.dump(ray_output_data, f, indent=2)
    
    with open(medium_file, "w") as f:
        json.dump(medium_output_data, f, indent=2)
    
    print(f"Datos de rayos guardados en: {ray_file}")
    print(f"Tamaño del archivo de rayos: {os.path.getsize(ray_file) / 1024 / 1024:.2f} MB")
    print(f"Datos del medio guardados en: {medium_file}")
    print(f"Tamaño del archivo del medio: {os.path.getsize(medium_file) / 1024 / 1024:.2f} MB")
